Feature: Scores - NBA
  ...
  @TEST_QA-6834 @applitools
  Scenario: Verify NBA Completed Gamecast UI for odds tab
    ...
    Then the "NBA COMPLETED - ODDS" scores screen is displayed
  ...
  @TEST_QA-6834 @no_game
  Scenario: Verify NBA Upcoming Gamecast UI for odds label
    Then I tap on the future game date
    And I view a "upcoming" gamecast
    Then I scroll down to verify odds partner label
    Then I scroll down to verify bet_play_now button

  @TEST_QA-7797 @no_game
  Scenario: Verify NBA odds UI validation on the gamecast track
    Then I tap on the previous game date
    And I view a "completed" gamecast
